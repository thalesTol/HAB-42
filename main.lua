...
--require 'mobdebug'
--require('mobdebug').start("192.168.100.5")

print("TEMPOO",os.time())
...
pdf:AddPage()
--image_name, x, y, w, h,atype, link,image_blob, blob_size
pdf:Image({
	image_name = "imagemErro.jpg",
	w = 200,
	h = 200,
	path = system.pathForFile(nil,system.TemporaryDirectory)
});
pdf:Output("impressaoImagens.pdf")
]]
----------------------
...
local licensingInit = nil
if system.getInfo("platform") == "android" then
	licensing = require( "licensing" )
 
	function licensingListener( event )
...
system.setIdleTimer( false )
	...
	local tipoSistema = checarPortatil()
	
	local vetorPastasAndroid = {}
	if tipoSistema == "android" then
		if auxFuncs.hasPhonePermission("android.permission.WRITE_EXTERNAL_STORAGE") then--"android.permission.WRITE_EXTERNAL_STORAGE"
			android.buscarAmazenamentosAndroid(vetorPastasAndroid)
		end
	end
	...
	local function lerArquivoAndroidParaLogin()
		varGlobais.vetorAndroidLogin = android.lerArquivoCodificadoAndroid({caminho = vetorPastasAndroid.armazenamentoInterno.."/Download",arquivo = "arquivoLoginLibEA.txt"})
		-- pegar os dados do arquivo android para que seja realizado o login no livro
		
		for j=1,#varGlobais.vetorAndroidLogin do
			--testarNoAndroid(varGlobais.vetorAndroidLogin[j],50 + 30*j)
			if varGlobais.vetorAndroidLogin[j] ~= nil then
				
				if string.find( varGlobais.vetorAndroidLogin[j]:match("([^=]+)="), "login" ) ~= nil then
					local aux = nil
					local lixo,str = varGlobais.vetorAndroidLogin[j]:match("([^=]+)=+([^\13]+)")
					for past in string.gmatch(str, '%s*(.+)') do
						aux = past
					end
					varGlobais.vetorAndroidLogin.login = aux
				end
				if string.find( varGlobais.vetorAndroidLogin[j]:match("([^=]+)="), "senha" ) ~= nil then
					local aux = nil
					local lixo,str = varGlobais.vetorAndroidLogin[j]:match("([^=]+)=+([^\13]+)")
					for past in string.gmatch(str, '%s*(.+)') do
						aux = past
					end
					varGlobais.vetorAndroidLogin.senha = aux
				end
				if string.find( varGlobais.vetorAndroidLogin[j]:match("([^=]+)="), "email" ) ~= nil then
					local aux = nil
					local lixo,str = varGlobais.vetorAndroidLogin[j]:match("([^=]+)=+([^\13]+)")
					for past in string.gmatch(str, '%s*(.+)') do
						aux = past
					end
					varGlobais.vetorAndroidLogin.email = aux
				end
				if string.find( varGlobais.vetorAndroidLogin[j]:match("([^=]+)="), "nome" ) ~= nil then
					local aux = nil
					local lixo,str = varGlobais.vetorAndroidLogin[j]:match("([^=]+)=+([^\13]+)")
					for past in string.gmatch(str, '%s*(.+)') do
						aux = past
					end
					varGlobais.vetorAndroidLogin.nome = aux
				end
				if string.find( varGlobais.vetorAndroidLogin[j]:match("([^=]+)="), "ID" ) ~= nil then
					local aux = nil
					local lixo,str = varGlobais.vetorAndroidLogin[j]:match("([^=]+)=+([^\13]+)")
					for past in string.gmatch(str, '%s*(.+)') do
						aux = past
					end
					varGlobais.vetorAndroidLogin.id = aux
				end
			end
		end
	...
	-- esconder a barra de baixo do android:
	
	-- Removes bottom bar on Android 
	local temUI = native.getProperty( "androidSystemUiVisibility" )
	print(temUI)
	--testarNoAndroid(temUI,100)
	...
	--testarNoAndroid("print(temUI)" .. tostring(temUI),200)
	...
	function funcGlobais.loadTable( filename, location )
	...
	function funcGlobais.saveTable( t, filename, location )
	...
	local function RegistrarHistorico(conteudo)
		...
		local vetorJsonAux = funcGlobais.loadTable("historico.json")
	...
	local function lerTextoVetorDoc(arquivo)
		local path = system.pathForFile( arquivo, system.DocumentsDirectory )	
	...
	if fileExistsDoc("paginaSalva.txt") then
		local paginaInicial = lerTextoDoc("paginaSalva.txt")
	...
	if fileExistsDoc("paginaVoltar.txt") then
		paginasParaVoltar = lerTextoVetorDoc("paginaVoltar.txt")
	end
	...
	function varGlobais.lerPreferenciasAutor()
		...
		local function lerPreferenciasLivroBanco()
			local function listenerLivro(event)
				if ( event.isError ) then
					if not fileExistsDoc("idLivro.txt") and varGlobais.temLogin then
	...
	local function escreverNoHistorico(texto,vetorJson)
		if varGlobais.temHistorico == true then
			...
			if vetorJson then
				...
				funcGlobais.saveTable( vetorJsonAux, "historico.json")
			end
			
			if fileExistsDoc("historico offline.txt") then
	...
	elseif varGlobais.temLogin == true then
		...
		local function telaDeLogin()
			...
			local function erroLoginDuplo(livro,IDlogin)
					...
					local function removerTelaLoginParaAcessoAutomatico()
						...
						if varGlobais.temHistorico == true and fileExistsDoc("historico.txt") then
							--uploadHistorico("historico "..Login0 .. " " .. EmailLogin0 .. ".txt")
							local conteudo = lerTextoDoc("historico.txt")
			...
			function tentarLoginAux()
				if fileExistsDoc("idAlunoEfetuado.txt") then
					varGlobais.idAluno0 = lerTextoDoc("idAlunoEfetuado.txt")
				
					if fileExistsDoc("nomeEfetuado.txt") then
						varGlobais.NomeLogin0 = lerTextoDoc("nomeEfetuado.txt")
					end
					if fileExistsDoc("apelidoEfetuado.txt") then
						varGlobais.apelidoAluno0 = lerTextoDoc("apelidoEfetuado.txt")
					end
					if fileExistsDoc("emailEfetuado.txt") then
						varGlobais.EmailLogin0 = lerTextoDoc("emailEfetuado.txt")
					end
					if fileExistsDoc("loginEfetuado.txt") then
						varGlobais.Login0 = lerTextoDoc("loginEfetuado.txt")
					end
					...
					if fileExistsDoc  ("idLivro.txt") then
						varGlobais.idLivro0 = lerTextoDoc("idLivro.txt")
					end
					local function realizarLoginAposVerificacaoDeAcesso()
						local grupoTravarTela = display.newGroup()
						local telaPreta = display.newRect(0,0,W,H)
		...
		if vetorPastasAndroid.armazenamentoInterno and android.fileExist(vetorPastasAndroid.armazenamentoInterno.."/Download","arquivoLoginLibEA.txt") then
	...
	--testarNoAndroid(vetorPastasAndroid.armazenamentoInterno.."/Download",100)
	
	
	--android.criarPastaAndroid({caminho = vetorPastasAndroid.armazenamentoInterno,pasta = "1pasta"})
